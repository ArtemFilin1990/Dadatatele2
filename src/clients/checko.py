...
class CheckoClient:
    ...
    def __init__(self, api_key: str, cache: TTLCache) -> None:
    ...
    async def fetch_finance(self, inn: str) -> dict[str, Any] | None:
        return await self._call("/v2/finance", {"inn": inn}, ttl=7 * 86400)
    ...
    async def _call(self, path: str, params: dict[str, str], ttl: int) -> dict[str, Any] | None:
        ...
        cache_key = f"checko:{path}:{urlencode(sorted(query.items()))}"
