...
class AggregatorService:
    def __init__(self, checko: CheckoClient, dadata: DaDataClient, refs: ReferenceDataService) -> None:
        self.checko = checko
        self.dadata = dadata
        self.refs = refs
    ...
    async def build_profile(self, inn: str) -> tuple[Profile | None, dict[str, Any]]:
        ...
        checko_address = _as_str(checko_data.get("ЮрАдрес") or checko_data.get("address"), "")
        dadata_address = _as_str((dadata_data.get("address") or {}).get("unrestricted_value"), "")
        checko_status = _as_str(checko_data.get("Статус"), "")
        dadata_status = _as_str((dadata_data.get("state") or {}).get("status"), "")

        conflict_parts: list[str] = []
        if checko_name and dadata_name and checko_name != dadata_name:
            conflict_parts.append(f"name: Checko='{checko_name}', DaData='{dadata_name}'")
        if checko_address and dadata_address and checko_address != dadata_address:
            conflict_parts.append("address: данные отличаются")
        if checko_status and dadata_status and checko_status != dadata_status:
            conflict_parts.append(f"status: Checko='{checko_status}', DaData='{dadata_status}'")
        ...
        profile = Profile(
            ...
            status=checko_status or dadata_status or "—",
            ...
            address=checko_address or dadata_address or "—",
            ...
            staff=_as_str(checko_data.get("Штат") or (checko_data.get("Персонал") or {}).get("Штат"), "н/д"),
            avg_salary=_as_str((checko_data.get("Персонал") or {}).get("СрЗарплата"), "н/д"),
            okved=okved_code,
            okved_title=self.refs.okved_name(okved_code),
            contacts=contacts,
            successor=_as_str(checko_data.get("Правопреемник") or "", ""),
            source_dispute=bool(conflict_parts),
            conflict_details="; ".join(conflict_parts),
        )
        return profile, {"entity": entity, "checko": checko_payload, "dadata": dadata_payload}
    ...
    def _merge_contacts(checko_data: dict[str, Any], dadata_data: dict[str, Any]) -> list[str]:
        ...
        phones_raw = contacts.get("Тел")
        phones_list = phones_raw if isinstance(phones_raw, list) else [phones_raw]
        for raw in phones_list:
            if raw:
        ...
        emails_raw = contacts.get("Емэйл")
        emails_list = emails_raw if isinstance(emails_raw, list) else [emails_raw]
        emails: list[str] = [str(e).strip().lower() for e in emails_list if e]
